...
修复历史:
  v0.7.1 之前:
    ...
    - 结果:`pip install` 后 site-packages/pandax/ 缺 installer/
            + `pandax install-context` 报"脚本不存在"

  v0.7.1 修复:
    - installer/ 移入 src/pandax/installer/(单路径,与 cli.py 同包)
    - pyproject.toml package-data 声明所有平台 installer/*
    - MANIFEST.in 路径同步更新为 src/pandax/installer/
    - cli.py INSTALLER_DIR = parent / "installer"(单路径,两场景通用)

验证:
  1. installer/ 在 src/pandax/ 下(dev 模式)
  2. pyproject.toml package-data 包含所有平台 installer/*
  3. _find_installer_dir() 在 dev 模式下返回正确路径
  4. mock 模拟 site-packages 模式也能找到
  5. 端到端: site-packages 安装下,`pandax install-context --help` 不报脚本缺失
  6. 端到端: site-packages 安装下,site-packages/pandax/installer/ 存在
"""
import os
import subprocess
import sys
...
PANDAX_SRC = ROOT / "src" / "pandax" / "cli.py"
PANDAX_DEV_INSTALLER = ROOT / "src" / "pandax" / "installer"  # v0.7.1+ 新位置
...
SITE_PANDAX = TRAE_PYTHON.parent / "lib" / "site-packages" / "pandax"
SITE_PANDAX_INSTALLER = SITE_PANDAX / "installer"


# ============================================================
# 测试 1: installer/ 在 src/pandax/ 下(结构正确)
# ============================================================
def test_installer_in_pandax_package():
    """v0.7.1 修复:installer/ 必须位于 src/pandax/installer/。"""
    assert PANDAX_DEV_INSTALLER.exists(), \
        f"installer/ 必须位于 src/pandax/installer/,实际: {PANDAX_DEV_INSTALLER}"
    # 三个平台都应存在
...
def test_find_installer_dir_function_exists():
    ...
    spec = importlib.util.spec_from_file_location("pandax_cli_test", PANDAX_SRC)
...
def test_find_installer_dir_dev_mode_works():
    """dev 模式下,能定位到 src/pandax/installer/。"""
    import importlib.util
    spec = importlib.util.spec_from_file_location("pandax_cli_test", PANDAX_SRC)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    ...
    # 应是 src/pandax/installer/,不是 site-packages
...
def test_find_installer_dir_installed_mode_works():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        # 模拟 site-packages 结构:tmp/site-packages/pandax/cli.py + tmp/site-packages/pandax/installer/
        fake_pandax = tmp_path / "site-packages" / "pandax"
        fake_pandax.mkdir(parents=True)
        (fake_pandax / "cli.py").write_text("# mock", encoding="utf-8")
        fake_installer = fake_pandax / "installer"
        ...
        # 替换 __file__ 让函数认为自己是 site-packages/pandax/cli.py
        with patch.object(mod, "__file__", str(fake_pandax / "cli.py")):
            result = mod._find_installer_dir()
            # 应返回 fake_pandax/installer(同 parent 关系)
...
def test_pyproject_package_data_includes_installer():
    ...
    assert "pandax" in content, "应有 pandax package-data 配置"
...
def test_manifest_in_installer_paths_updated():
    """MANIFEST.in 的 installer 路径必须指向 src/pandax/installer/。

    旧路径(installer/windows/*.ps1)会让 wheel 不包含 installer。
    新路径(src/pandax/installer/...)必须存在。
    """
    manifest = (ROOT / "MANIFEST.in").read_text(encoding="utf-8")
    # 新的相对路径必须出现
    assert "src/pandax/installer" in manifest, \
        "MANIFEST.in 应使用 src/pandax/installer/ 路径"
    # 旧的项目根路径不应再出现(只有相对项目根的 src/pandax/...)
    # 找独立的 "include installer/" 或 "recursive-include installer/" 模式
    import re
    bad_patterns = re.findall(r"^(?!.*src/pandax).*\binstaller[/\\]", manifest, re.MULTILINE)
    # 排除带 src/ 前缀的合法行
    bad_lines = [
        line for line in manifest.splitlines()
        if ("installer" in line or "installer/" in line or "installer\\" in line)
        and "src/pandax/installer" not in line
        and not line.strip().startswith("#")
    ]
    assert not bad_lines, f"MANIFEST.in 仍含项目根级 installer 路径(应改为 src/pandax/installer):\n" + "\n".join(bad_lines)
...
@pytest.mark.skipif(
    ...
    reason="site-packages/pandax/ 不存在(无 TRAE Python 或未安装),跳过 e2e"
)
def test_installed_pandax_has_installer_in_site_packages():
    """site-packages/pandax/installer/ 必须存在(由 package-data 决定)。"""
    assert SITE_PANDAX_INSTALLER.exists(), (
        f"site-packages/pandax/installer/ 必须存在(Bug A 已修),"
        f"实际: {SITE_PANDAX_INSTALLER}"
    )
    if sys.platform == "win32":
        assert (SITE_PANDAX_INSTALLER / "windows" / "install_context_menu.ps1").exists(), \
            "site-packages/pandax/installer/windows/install_context_menu.ps1 必须存在"
        assert (SITE_PANDAX_INSTALLER / "windows" / "uninstall_context_menu.ps1").exists(), \
            "site-packages/pandax/installer/windows/uninstall_context_menu.ps1 必须存在"
...
def test_installed_pandax_install_context_help_works():
    """site-packages 装的 pandax 跑 `pandax install-context --help` 不能报 installer 缺失。"""
    result = subprocess.run(
        [str(TRAE_PYTHON), "-m", "pandax", "install-context", "--help"],
