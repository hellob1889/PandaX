...
- 验证 unlock 时被正确清理
- 验证 ICO 文件被正确复制到 .pandaone/icon/
- 验证 desktop.ini 内容引用相对路径(卸载 pandaone 后图标仍能显示)
...
class TestDesktopIconUnit:
    ...
    def test_read_ico_from_dev_path(self, tmp_path: Path):
        """开发模式:ICO 在 src/pandaone/ 下,应能读取。"""
        from pandaone.desktop_icon import _read_ico_from_package
        # 实际 ICO 在 src/pandaone/ 下,直接读
    ...
    def test_apply_creates_desktop_ini(self, tmp_path: Path):
        ...
        from pandaone.desktop_icon import apply_desktop_icon
        # 模拟 Windows 平台
        with patch("pandaone.desktop_icon.sys") as mock_sys:
    ...
    def test_apply_uses_relative_path(self, tmp_path: Path):
        ...
        # 必须包含 .pandaone\icon\pandax_locked.ico(相对)
        assert ".pandaone" in content
        assert "icon" in content
        # 不能包含绝对路径驱动器
        assert ":\\" not in content or "IconFile=.pandaone" in content

    def test_apply_copies_ico_to_pandaone_dir(self, tmp_path: Path):
        """ICO 复制到 .pandaone/icon/ 而不是污染用户目录。"""
        from pandaone.desktop_icon import apply_desktop_icon
        with patch("pandaone.desktop_icon.sys") as mock_sys:
            mock_sys.platform = "win32"
            apply_desktop_icon(tmp_path, "locked")
        icon_path = tmp_path / ".pandaone" / "icon" / "pandax_locked.ico"
    ...
    def test_apply_unlocked_uses_correct_ico(self, tmp_path: Path):
        ...
        assert (tmp_path / ".pandaone" / "icon" / "pandax_unlocked.ico").exists()
    ...
    def test_remove_cleans_desktop_ini(self, tmp_path: Path):
        ...
        assert not (tmp_path / ".pandaone" / "icon").exists(), ".pandaone/icon 未清理"

    def test_remove_idempotent(self, tmp_path: Path):
        """无 desktop.ini 时 remove 也是 idempotent。"""
        from pandaone.desktop_icon import remove_desktop_icon
        with patch("pandaone.desktop_icon.sys") as mock_sys:
            mock_sys.platform = "win32"
            ok, msg = remove_desktop_icon(tmp_path)
        assert ok is True
    ...
    def test_has_desktop_icon(self, tmp_path: Path):
        ...
        from pandaone.desktop_icon import apply_desktop_icon, has_desktop_icon
        with patch("pandaone.desktop_icon.sys") as mock_sys:
            mock_sys.platform = "win32"
...
class TestLockUnlockWithIcon:
    ...
    @pytest.fixture
    def pandaone_root(self, tmp_path: Path):
        """创建已 init 的 pandaone 临时目录。"""
        import subprocess
        ...
        assert rc == 0
        # 验证 .pandaone/config.json 已创建
        assert (tmp_path / ".pandaone" / "config.json").exists()
        return tmp_path

    def test_lock_creates_desktop_ini(self, pandaone_root: Path):
        """pandaone lock 应自动创建 desktop.ini。"""
        with patch("pandaone.desktop_icon.sys") as mock_sys:
            mock_sys.platform = "win32"
            from pandaone.cli import cmd_lock
            args = MagicMock(root=str(pandaone_root), no_auto_init=True)
            rc = cmd_lock(args)
        assert rc == 0
        assert (pandaone_root / "desktop.ini").exists()
        assert (pandaone_root / ".pandaone" / "icon" / "pandax_locked.ico").exists()

    def test_unlock_removes_desktop_ini(self, pandaone_root: Path):
        """pandaone unlock 应清理 desktop.ini。"""
        with patch("pandaone.desktop_icon.sys") as mock_sys:
            mock_sys.platform = "win32"
            from pandaone.cli import cmd_lock, cmd_unlock
            # 先 lock
            cmd_lock(MagicMock(root=str(pandaone_root), no_auto_init=True))
            assert (pandaone_root / "desktop.ini").exists()
            # 再 unlock
            cmd_unlock(MagicMock(root=str(pandaone_root), no_auto_init=True))
            assert not (pandaone_root / "desktop.ini").exists()

    def test_lock_icon_failure_does_not_block(self, pandaone_root: Path):
        """图标切换失败不应阻塞 lock 主流程(对抗式审查)。"""
        from pandaone.cli import cmd_lock
        # 模拟图标写入失败
        with patch("pandaone.desktop_icon.apply_desktop_icon",
...
class TestICOFiles:
    ...
    def test_ico_visual_distinct(self):
        ...
        from PIL import Image
        from pathlib import Path
        locked = Image.open(Path(__file__).parent.parent / "src" / "pandaone" / "pandax_locked.ico")
        unlocked = Image.open(Path(__file__).parent.parent / "src" / "pandaone" / "pandax_unlocked.ico")
        # 256x256 主图逐像素比对
