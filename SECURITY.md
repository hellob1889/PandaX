...
Pandaone AI Agent 是**代码审计门禁工具**，安全是它的核心职责。本文档说明如何报告 Pandaone 自身的安全漏洞，以及我们的安全更新策略。
...
1. **GitHub Security Advisories（推荐）**
   https://github.com/hellob1889/Pandaone-AI-Agent/security/advisories/new
   - 私密沟通，可协调披露时间
   - 允许我们在补丁准备好前隐藏报告
...
- **GitHub Security Advisories**：https://github.com/hellob1889/Pandaone-AI-Agent/security/advisories
- **PyPI Release History**：https://pypi.org/project/pandaone-guard/#history
- **GitHub Releases**：https://github.com/hellob1889/Pandaone-AI-Agent/releases

订阅 GitHub Releases 的 "Watch → Releases only" 即可收到所有版本通知（含安全补丁）。

## Pandaone 自身的安全模型

Pandaone 提供 **7 层防御体系**，自身安全等同于其要保护的项目：

| 层 | 安全相关组件 | 已知限制 |
|---|------|------|
| L1 | 文件 chmod 只读锁 | 仅限本地用户，root/管理员可绕过 |
| L2 | watchdog 实时监控 | 进程级，单实例；不支持网络文件系统 |
| L3 | pre-commit hook | 仅本地 hook，CI 端需要额外的 `pandaone ci` 检查 |
| L5 | 自指纹 SHA256 | 文件级，wheel 构建时已强制 LF normalize（避免 CRLF 漂移） |
| L7 | GitHub Actions CI | PR 合入前审计；无法防止本地 bypass |

**重要**：Pandaone 是**威慑 + 审计**工具，不是**不可绕过**的强制访问控制。恶意开发者可以通过修改 Pandaone 自身代码绕过所有层。Pandaone 的核心价值是**事后可追溯性**（审计日志），而非**事前强制阻止**。

## Out of Scope（不在漏洞披露奖励范围内）

- Pandaone 本身的依赖（click / pyyaml / openpyxl 等）漏洞 —— 请报告给上游
- 在未启用 Pandaone 的项目中的代码泄露
- 社会工程学 / 钓鱼攻击
- DoS / 暴力破解 Pandaone CLI 本身
