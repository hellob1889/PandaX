...
第一性原理：
  ...
  - 每个工具 = 一个 CLI 子命令的封装，复用现有 pandax.py
...
TOOLS = [
    {
        "name": "pandax_init",
        "description": "在指定目录初始化 PandaX（创建 .pandax/、config.json、pandax.jsonl、可选 binary_snapshots.json）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "pandax_fingerprint_update",
        "description": "更新 pandax 自身的 SHA256 指纹（合法修改 CLI源码后必须调用）",
        "inputSchema": {
            "type": "object",
            "properties": {
...
def _run_cli(args: list, timeout: int = 30) -> dict:
    """调用 pandax CLI 子进程，返回 {returncode, stdout, stderr}"""
...
HANDLERS = {
    "pandax_init": lambda p: _run_cli(["init", *_args_from(p, [
        ("root", "--root"), ("ext", "--ext"), ("no_binary", "--no-binary")])], timeout=60),
    "pandax_lock": lambda p: _run_cli(["lock", *_args_from(p, [("root", "--root")])]),
    "pandax_unlock": lambda p: _run_cli(["unlock", *_args_from(p, [("root", "--root")])]),
    "pandax_write": lambda p: _run_cli(["write", *_args_from(p, [
        ("root", "--root"), ("file", "--file"),
    ...
    "pandax_log": lambda p: _run_cli(["log", *_args_from(p, [
