...
def cmd_log(args):
    """
    查询审计历史（stdout 输出）。
    导出请用 `pandaone export --format <fmt> --output <path>`。
    """
...
def cmd_export(args):
    ...
    用法: pandaone export --format html --output report.html
         pandaone export --format json -o data.json --rejected
    """
    filtered, root = _query_records(args)
    if root is None:
        return 1
...
def cmd_status(args):
    ...
    # Bug #16 fix
    try:
        config = json.loads((pandaone_dir / "config.json").read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError) as e:
        print(t("err_config_corrupted", err=e))
        return 1
    ...
    print()
    # Git 集成状态(v0.7.1+方案A):检查 .gitignore 是否正确排除 Pandaone 自动产物
    try:
        from pandaone.gitignore_helper import is_git_repo, check_gitignore
        if is_git_repo(root):
            ok, missing = check_gitignore(root)
            if ok:
                print(t("status_gitignore_ok") if "status_gitignore_ok" in dir(__builtins__) else "[OK] Git 集成:.gitignore 已正确排除 .pandaone/ + desktop.ini")
            else:
                miss_names = [e for e in missing if not e.startswith("#")]
                print(t("status_gitignore_missing", miss=", ".join(miss_names)) if "status_gitignore_missing" in dir(__builtins__) else f"[WARN] Git 集成:.gitignore 缺失 {miss_names} → 重跑 pandaone init 自动修复")
    except ImportError:
        pass
    print()
...
def cmd_serve(args):
    ...
    第一性原理:
      ...
      - watchdog 监视 .pandaone/pandaone.jsonl → SSE 推送到浏览器
      - 浏览器渲染现代明亮仪表盘,Notion/Linear 风格

    用法:
      pandaone serve --root .           # 启动 + 自动打开浏览器
      pandaone serve --port 9000        # 自定义端口
      pandaone serve --no-browser       # 不自动打开(适合远程/CI)
...
def cmd_install_hook(args):
    ...
    try:
        ...
        print(t("warn_subprocess_timeout", cmd="pandaone.install_hook", timeout=10))
        return 1
...
def cmd_watch(args):
    ...
    if getattr(args, "daemon", False):
        ...
        # PyInstaller 环境下用打包后的 exe；pip 安装后用 python -m pandaone_guard；开发模式用 .py
        if getattr(sys, "frozen", False):
            # 打包环境：pandaone_guard.exe 在 pandaone.exe 旁边
            ...
            # 优先用 python -m pandaone_guard（pip 安装后）
    ...
    # 前台模式：直接调用 pandaone_guard.run_watchdog()
...
def cmd_ci(args):
    ...
    检查 base..head 之间所有变更是否都有审计：
      ...
      3. 每个文本文件：检查 pandaone.jsonl 是否有 APPROVED 记录
    ...
    对抗式审查：
      - 攻击：agent 改 pandaone.jsonl 抹除痕迹
        缓解：CI 比较 git 历史里的 audit 文件
      - 攻击：删 .pandaone/ 抹除全部记录
        ...
        if ext in binary_exts:
            ...
            if rel not in approved_files:
                violations.append({
                    "file": rel,
                    "type": "text",
                    "reason": "未找到 APPROVED 审计记录（可能绕过 pandaone write 直接修改）",
                })
...
def _find_installer_dir() -> Path:
    ...
    自 v0.7.1+ 起,installer/ 已移入 src/pandaone/installer/,打包后位于
    site-packages/pandaone/installer/。两种场景使用同一路径:
        - site-packages 安装:site-packages/pandaone/installer/
        - dev 模式 (PYTHONPATH=src):<project>/src/pandaone/installer/
    """
...
def _run_installer(action: str, force: bool = False):
    ...
    # 传当前语言给子进程（installer 脚本读 ~/.pandaone/config.json 即可）
...
def cmd_install_context(args):
    ...
    Phase 10: --lang 指定 UI 语言（写入 ~/.pandaone/config.json，被 PS1 脚本读取）
    """
    force = getattr(args, "force", False)
    lang = getattr(args, "lang", None)
    if lang:
        # 先切换语言（写入 ~/.pandaone/config.json）让 PS1 脚本读取
        i18n_init(lang)
...
def main(argv=None):
    ...
    # 导致 `pandaone status --lang=en --root .` 不生效。手动预提取后塞回 args.lang。
    import re as _re
    ...
    # 优先级：--lang > 用户偏好 ~/.pandaone/config.json > OS 自动检测
    i18n_init(args.lang)
    ...
    if "--update-fingerprint" in argv_list:
        ...
        # 目的：让普通用户不需要手动跑 `pandaone --update-fingerprint 0000`
