#!/usr/bin/env bash
# ============================================================
# Pandaone AI Agent Linux right-click menu installer
# ============================================================
#
# Usage:
...
#   pandaone install-context              # uses ~/.pandaone/config.json
...
if [ "$USER_LANG" = "en" ]; then
    TXT_TITLE="Pandaone Linux Context Menu Installer"
    TXT_NOT_FOUND="[WARN] pandaone not found! Please run 'pip install pandaone-guard' first"
    ...
    TXT_USAGE_NAUTILUS_1="    right-click -> Scripts -> Pandaone -> choose action"
    TXT_USAGE_DOLPHIN="  Dolphin (KDE):"
    TXT_USAGE_DOLPHIN_1="    right-click -> Actions -> Pandaone -> choose sub-action"
    ...
    TXT_TITLE="Pandaone Linux 右键菜单安装程序"
    TXT_NOT_FOUND="[WARN] 未找到 pandaone！请先 pip install pandaone-guard"
    ...
    TXT_USAGE_NAUTILUS="  Nautilus（Files）:"
    TXT_USAGE_NAUTILUS_1="    右键 → 脚本 → Pandaone → 选择操作"
    TXT_USAGE_DOLPHIN="  Dolphin（KDE）:"
    TXT_USAGE_DOLPHIN_1="    右键 → Actions → Pandaone → 选择子操作"
...
NAUTILUS_SRC="$SCRIPT_DIR/nautilus/Pandaone"
DOLPHIN_SRC="$SCRIPT_DIR/dolphin/pandaone-lock.desktop"
...
if ! command -v pandaone >/dev/null 2>&1; then
    ...
    echo "$TXT_FOUND $(command -v pandaone)"
fi
...
case "$DE" in
    *gnome*|*cinnamon*|*mate*|*unity*|*pop*)
        ...
        # 拷贝 + 赋权
        cp "$NAUTILUS_SRC" "$NAUTILUS_DST_DIR/Pandaone"
        chmod +x "$NAUTILUS_DST_DIR/Pandaone"
        echo "$TXT_INSTALLED $NAUTILUS_DST_DIR/Pandaone"
...
case "$DE" in
    *kde*|*plasma*)
        ...
        cp "$DOLPHIN_SRC" "$DOLPHIN_DST_DIR/pandaone-lock.desktop"
        chmod +x "$DOLPHIN_DST_DIR/pandaone-lock.desktop"
        ...
        echo "$TXT_INSTALLED $DOLPHIN_DST_DIR/pandaone-lock.desktop"
        installed=$((installed + 1))
        ;;
esac
