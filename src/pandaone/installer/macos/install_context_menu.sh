#!/usr/bin/env bash
# ============================================================
# Pandaone AI Agent macOS context menu installer
# ============================================================
#
# Usage:
#   bash installer/macos/install_context_menu.sh
#
# Or via pandaone (preferred, sets --lang):
#   pandaone install-context              # uses ~/.pandaone/config.json (set by pandaone)
#   pandaone install-context --lang=en    # overrides saved preference
#
# First principles:
#   - macOS "Services" (Quick Actions) live in ~/Library/Services/ as .workflow dirs
#   - Finder right-click -> Services -> Pandaone -> choose action -> Terminal runs pandaone
...
#
# i18n: language comes from ~/.pandaone/config.json (written by `pandaone --lang`),
...
if [ -z "$USER_LANG" ] && [ -f "$HOME/.pandaone/config.json" ]; then
    USER_LANG=$(grep -o '"lang":[[:space:]]*"[^"]*"' "$HOME/.pandaone/config.json" 2>/dev/null \
                | head -1 | sed 's/.*"\([^"]*\)"$/\1/' || echo "")
fi
...
if [ "$USER_LANG" = "en" ]; then
    TXT_TITLE="Pandaone macOS Context Menu Installer"
    TXT_NOT_FOUND="[WARN] pandaone command not found!"
    TXT_INSTALL_HINT_1="Please install first:"
    TXT_INSTALL_HINT_2="  pip3 install pandaone-guard"
    TXT_INSTALL_HINT_3="Or:"
    TXT_INSTALL_HINT_4="  pip3 install --user git+https://github.com/hellob1889/Pandaone-AI-Agent.git"
    TXT_CONFIRM="Continue anyway? [y/N]"
    TXT_CANCELLED="Cancelled."
    TXT_FOUND="[OK] Found pandaone:"
    TXT_NO_WORKFLOW="[ERROR] Workflow source not found:"
    TXT_BAD_ID="[ERROR] Workflow identifier mismatch (expected com.pandaone.workflow.lock, got"
    ...
    TXT_NEXT_2="  Check 'Files and Folders' -> 'Pandaone'"
    TXT_USAGE="After enabling:"
    TXT_USAGE_1="  Finder -> right-click file/folder -> Services -> Pandaone"
    TXT_USAGE_2="  -> choose init / lock / unlock / status"
    TXT_USAGE_3="  -> Terminal opens automatically and runs pandaone"
    TXT_UNINST="To uninstall:"
    TXT_UNINST_CMD="  bash installer/macos/uninstall_context_menu.sh"
else
    TXT_TITLE="Pandaone macOS 右键服务安装程序"
    TXT_NOT_FOUND="[WARN] 未找到 pandaone 命令！"
    ...
    TXT_CANCELLED="已取消。"
    TXT_FOUND="[OK] 找到 pandaone:"
    TXT_NO_WORKFLOW="[ERROR] 未找到 workflow 源:"
    TXT_BAD_ID="[ERROR] workflow 标识不符（期望 com.pandaone.workflow.lock，实际"
    ...
    TXT_NEXT_2="  勾选「文件和文件夹」→「Pandaone」"
    TXT_USAGE="启用后使用方式："
    TXT_USAGE_1="  Finder → 右键点击文件/文件夹 → 服务 → Pandaone"
    TXT_USAGE_2="  → 选择 init / lock / unlock / status"
    TXT_USAGE_3="  → Terminal 自动打开并执行 pandaone 命令"
    TXT_UNINST="卸载："
    TXT_UNINST_CMD="  bash installer/macos/uninstall_context_menu.sh"
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKFLOW_SRC="$SCRIPT_DIR/Pandaone Lock.workflow"
...
# 1. 检查 pandaone 是否可用
if ! command -v pandaone >/dev/null 2>&1; then
    echo "$TXT_NOT_FOUND"
    ...
    PANDAX_PATH="$(command -v pandaone)"
...
if [ "$PLIST_ID" != "com.pandaone.workflow.lock" ]; then
...
case "$USER_LANG" in
    en)
        ...
        WF_COMMENT_TERMINAL="Execute pandaone in Terminal.app"
    ...
    *)
        ...
        WF_COMMENT_TERMINAL="在 Terminal.app 中执行 pandaone"
