"""维护项目 .gitignore,确保 Pandaone AI Agent 自动产物不被 git 跟踪。

第一性原理(为什么这个模块存在):
  - pandaone lock 会写入 `desktop.ini`(Windows 文件夹图标配置)
  - pandaone init 会创建 `.pandaone/` 目录(配置/审计/快照)
  - 这些文件**不应**被 git 跟踪:
    - `.pandaone/` 是运行时状态(每台机器可能不同,审计日志是个人数据)
...
对抗式审查:
  ...
  - 顺序敏感:.pandaone/ 必须先于 desktop.ini(虽然 gitignore 顺序不严格,但习惯)
...
def ensure_gitignore(root: Path, entries: Iterable[str] = REQUIRED_GITIGNORE_ENTRIES) -> tuple[bool, str]:
    ...
    # 拼接新条目(块头加注释说明)
    block = ["", "# Pandaone 锁定的标识文件(自动管理,不要 commit)"] if not any(
        "Pandaone" in line for line in existing_lines
    ) else [""]
    block.extend(to_add)
