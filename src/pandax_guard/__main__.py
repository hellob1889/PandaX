...
机制：
  ...
  - 守护进程模式：写 PID 到 .pandax/.watchdog_pid
  - 优雅退出（KeyboardInterrupt）

用法：
  python pandax_guard.py --root <project>           # 前台
  python pandax_guard.py --root <project> --daemon  # 后台
"""
import datetime
import json
import os
...
class PandaXHandler(FileSystemEventHandler):
    ...
    def __init__(self, root: Path):
        ...
        if config_path.exists():
            try:
                ...
                self.binary_snapshots_path = self.root / ".pandax" / "binary_snapshots.json"
    ...
    def _is_excluded(self, path: Path) -> bool:
        """排除 __pycache__、_tmp_*、.git、.pandax 等"""
        try:
...
def run_watchdog(root: Path, daemon: bool = False):
    ...
    pid_path = root / ".pandax" / ".watchdog_pid"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="PandaX L2 watchdog 守护进程")
