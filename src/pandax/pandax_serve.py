"""PandaX Web 仪表盘后端(watchdog + HTTP + SSE 三合一)。

第一性原理:
  - 用户想要"实时看到 Agent 修改的内容"
  - 审计日志写入 .pandax/pandax.jsonl(append-only)
  - 用 watchdog 监视文件 append → SSE 推送到浏览器
  - 浏览器渲染现代明亮仪表盘

架构:
  Agent write → .pandax/pandax.jsonl (append)
...
class _AuditFileHandler(FileSystemEventHandler if HAS_WATCHDOG else object):
    """watchdog handler:.pandax/pandax.jsonl append 时读取新行并发布事件。"""
    ...
    def on_modified(self, event):
        ...
        if event.is_directory:
            return
        # 只关心 pandax.jsonl(可能是其他文件变化,过滤掉)
...
class AuditFileMonitor:
    """启动 watchdog 监视 .pandax/pandax.jsonl,启动时回放历史 + 实时增量。"""

    def __init__(self, root: Path, bus: AuditEventBus):
        self.root = Path(root).resolve()
        self.audit_path = self.root / ".pandax" / "pandax.jsonl"
        self.bus = bus
        self._observer = None
        self._handler = None
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _serve_stats(self):
        ...
        audit_path = self.root / ".pandax" / "pandax.jsonl"
...
def start_server(root: Path, port: int = 8765, host: str = "127.0.0.1",
    ...
    try:
        ...
        _Handler.index_html = resources.files("pandax").joinpath(INDEX_HTML_NAME).read_bytes()
...
if __name__ == "__main__":
    # 命令行调试入口
    import argparse
    ap = argparse.ArgumentParser(description="PandaX Web 仪表盘")
    ap.add_argument("--root", default=".", help="pandax 项目根")
