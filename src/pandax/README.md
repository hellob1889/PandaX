...
- RED：写 `tests/test_readme_loaded.py` 共 4 个测试
  ...
  - `test_pandax_file_exists`：pandax.py 必须存在
    `test_pandax_file_exists`: pandax.py must exist
...
- 验证 RED：3 failed, 1 passed（pandax.py 不存在，失败原因正确）
  Verify RED: 3 failed, 1 passed (pandax.py doesn't exist, failure reason correct)
...
### Step 2 — 实现 pandax.py 框架 + 启动读 README / Step 2 — Implement pandax.py Framework + Startup Read README
...
- GREEN：写 `pandax.py`，所有 4 个测试通过
  GREEN: write `pandax.py`, all 4 tests pass
...
- `pandax.py`（v0.0.1，含 argparse 框架 + README 摘要 / with argparse framework + README summary）

***

### Step 3 — 实现 init 子命令 / Step 3 — Implement init Subcommand

**目标 / Goal**：让 `pandax init --root <path>` 在指定目录创建 `.pandax/` 基础设施。
Make `pandax init --root <path>` create `.pandax/` infrastructure in the specified directory.
...
- RED：写 `tests/test_init.py` 5 个测试
  ...
  - 创建 .pandax/ 目录 / Create .pandax/ directory
...
- 手测：`pandax init --root test_project` 成功创建 / Manual test: `pandax init --root test_project` creates successfully
...
- `pandax.py`（新增 init 子命令 + --root 参数 / added init subcommand + --root param）

***

### Step 4 — 实现 lock / unlock 子命令 / Step 4 — Implement lock / unlock Subcommands

**目标 / Goal**：让 `pandax lock / unlock --root <path>` 切换项目 .py 文件的只读状态。
Make `pandax lock / unlock --root <path>` toggle readonly state of project .py files.
...
- `pandax.py`（新增 lock / unlock 子命令 + _apply_readonly 共享函数 / added lock/unlock subcommands + _apply_readonly shared function）

***

### Step 5 — 实现 write 子命令（核心）/ Step 5 — Implement write Subcommand (Core)

**目标 / Goal**：让 `pandax write --file X --reason Y --problem Z --approach W` 完成完整审计写入流程。
Complete full audit write workflow via `pandax write --file X --reason Y --problem Z --approach W`.
...
- `pandax.py`（新增 write 子命令，含完整 8 步流程 / added write subcommand with complete 8-step flow）
...
- `--auto-download`：自动下载 PortableGit zip（约 50MB）到 `<pandax_dir>/git/` / Auto-download to <pandax_dir>/git/
...
- `pandax.py`（新增 install-git 子命令 / added install-git subcommand）
...
**目标 / Goal**：让 `pandax log` 支持查询、过滤、导出审计记录。
`pandax log` supports query, filter, export audit records.
...
- `pandax.py`（新增 log 子命令 + _print_log / _export_html 辅助函数 / added log subcommand + helper funcs）

***

### Step 7 — 实现 SHA256 自指纹保护（L5 防御）/ Step 7 — Implement SHA256 Self-Fingerprint (L5 Defense)

**目标 / Goal**：检测 pandax.py 自身被篡改，提供合法更新机制。
Detect pandax.py tampering; provide legitimate update mechanism.

**决策 / Decision**：

- 用 `hashlib.sha256(pandax.py.read_bytes())` 计算指纹 / Compute fingerprint via `hashlib.sha256`

- 存到 `.pandax_fp.txt`（项目根目录）/ Store at `.pandax_fp.txt` (project root)
...
- RED：写 `tests/test_fingerprint.py` 5 个测试 / RED: write `tests/test_fingerprint.py` 5 tests
  ...
  - 指纹内容 = pandax.py 的 SHA256 / Fingerprint content = SHA256 of pandax.py
...
- 攻击：agent 直接修改 .pandax_fp.txt / Attack: agent directly modifies .pandax_fp.txt
...
- `pandax.py`（新增指纹机制 + --update-fingerprint 参数 / added fingerprint mechanism + --update-fingerprint param）

- `.pandax_fp.txt`（生成的指纹文件 / generated fingerprint file）
...
1. init → .pandax/ 创建 / .pandax/ created
...
- 缓解：README 与 `pandax.py` 指纹无关；篡改 README 只能误导信息展示，不影响审计逻辑
  Mitigation: README unrelated to `pandax.py` fingerprint; tampering only misleads info display, not audit logic
...
├── PandaX_项目文档.html             # 原始设计文档（v2.0 / original design doc）
├── pandax.py                        # 主 CLI（待创建 / main CLI, to create）
├── pandax_guard.py                   # L2 监控（Phase 2 / L2 monitor）
├── install_hook.py                     # L3 hook 安装器（Phase 2 / L3 hook installer）
├── audit_viewer.py                     # 查询工具（可作为 log 后端 / query tool, can be log backend）
├── .pandax_fp.txt                   # pandax 自身指纹 / pandax self-fingerprint
├── templates/
...
│   ├── test_log.py
│   └── test_e2e.py
└── test_project/                       # 用 PandaX 保护的测试项目 / test project protected by PandaX
    ├── .pandax/                     # init 创建 / created by init
