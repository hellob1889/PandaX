...
pre-commit-check.py
===================
PandaX pre-commit hook 的 Python 校验逻辑（Phase 4.6+）。

由 .git/hooks/pre-commit 调用（shell 脚本只负责传参）。

Phase 10: i18n — 所有 print 走 t()，由 pandax.i18n 提供。
注意：pre-commit 脚本运行时可能没有安装 pandax，因此有 fallback 字符串。
...
def t_safe(key: str, **kwargs) -> str:
    """
    安全翻译函数（fallback）。
    如果 pandax 未安装，fallback 到内置中文字符串。
    """
    try:
        from pandax.i18n import t as _t
        ...
        fallback_zh = {
            "_hook_no_config": "[PandaX] 拒绝提交: .pandax/config.json 不存在",
            "_hook_init_hint": "请先运行: pandax init",
            "_hook_cfg_parse_err": "[PandaX] config.json 解析失败: {e}",
            "_hook_git_diff_err": "[PandaX] git diff 失败: {stderr}",
            "_hook_git_call_err": "[PandaX] git 调用失败: {e}",
            "_hook_no_audit": "[PandaX] 拒绝提交: 审计日志不存在",
            "_hook_should_exist": "应存在: {path}",
            "_hook_audit_read_err": "[PandaX] 读取审计日志失败: {e}",
            "_hook_reject_no_audit": "[PandaX] 拒绝提交: 以下文件没有 APPROVED 审计记录",
            "_hook_staged_files": "被 staged 的受保护文件:",
            "_hook_use_pandax_write": "请使用 pandax write 命令代替直接 git commit:",
            "_hook_write_example": '  pandax write --file <FILE> --reason "..." --problem "..." --approach "..."',
...
def main():
    repo_root = Path.cwd()
    config_path = repo_root / ".pandax" / "config.json"
    audit_path = repo_root / ".pandax" / "pandax.jsonl"
    ...
    if missing:
        ...
        for m in missing:
            print(f"  {m}")
        print("")
        print(t_safe("_hook_use_pandax_write"))
