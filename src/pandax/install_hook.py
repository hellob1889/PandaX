...
install_hook.py
===============
安装 PandaX pre-commit hook 到 .git/hooks/
...
def install_hook(root: Path) -> int:
    ...
    # Phase 4.6+: 同时复制 pre-commit-check.py 到 .pandax/（便于 hook 调用）
    pandax_dir = root / ".pandax"
    pandax_dir.mkdir(parents=True, exist_ok=True)
    if CHECK_SCRIPT.exists():
        shutil.copy(CHECK_SCRIPT, pandax_dir / "pre-commit-check.py")
    ...
    print(t("_hook_check_copied", path=pandax_dir / 'pre-commit-check.py'))
