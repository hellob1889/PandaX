# Pandaone AI Agent v0.7.1 安装与使用指南 / Pandaone v0.7.1 Installation & Usage Guide

> **历史归档**：v0.7.1 是第一次 PyPI 正式发布。修复了 v0.7.0 中 install-context 通配符阻塞 bug；首次使用 `pandaone-guard` 分发名（PyPI 黑名单导致）。行为与 v0.7.0/v0.7.2 完全一致，差异在 CI 工程化。
> **Historical archive**: v0.7.1 is the first official PyPI release. Fixed the v0.7.0 install-context wildcard block bug; first used the `pandaone-guard` distribution name (PyPI blacklist). Behavior fully compatible with v0.7.0/v0.7.2; differences only in CI engineering.
...
# 假设 Pandaone-0.7.1-install.zip 解压到 D:\pandaone-pkg\ / Assume Pandaone-0.7.1-install.zip extracted to D:\pandaone-pkg\
cd D:\pandaone-pkg

# 安装 wheel（pip 会自动拉依赖）/ Install wheel (pip auto-pulls dependencies)
pip install dist\pandaone_guard-0.7.1-py3-none-any.whl

# 或一行完成 / Or one-liner
pip install pandaone_guard-0.7.1-py3-none-any.whl
```

### 2.2 备选：从源码 sdist 安装 / Alternative: Install from Source sdist

```bash
pip install dist\pandaone_guard-0.7.1.tar.gz
```
...
```bash
pip download pandaone_guard-0.7.1-py3-none-any.whl -d deps/
# 会下载 watchdog / openpyxl / python-docx / reportlab / pyyaml 等
# Will download watchdog / openpyxl / python-docx / reportlab / pyyaml etc.

# 把 Pandaone-0.7.1-install.zip + deps/ 一起拷到目标电脑
# Copy Pandaone-0.7.1-install.zip + deps/ to target machine together
# 目标电脑上 / On target machine:
pip install --no-index --find-links=deps/ pandaone_guard-0.7.1-py3-none-any.whl
```
...
如果想直接用 `pandaone` 而不是 `python -m pandaone` / If you want to use `pandaone` directly instead of `python -m pandaone`:
```bash
...
# 2. 初始化 Pandaone（创建 .pandaone/ + 锁定受保护文件）/ Initialize Pandaone (creates .pandaone/ + locks protected files)
...
## 五、v0.7.1 包含的修复 / Fixes Included in v0.7.1
...
- **#25** 独立 `pandaone export` 子命令 / independent `pandaone export` subcommand
...
```bash
pip install --no-build-isolation dist\pandaone_guard-0.7.1.tar.gz
# 或 / Or
python -m build --no-isolation   # 在打包端 / on the packaging side
```

### Q2: Windows 上找不到 `pandaone` 命令 / Can't find `pandaone` command on Windows
A: 用 `python -m pandaone ...` 代替。或者把 Scripts 目录加到 PATH / Use `python -m pandaone ...` instead. Or add Scripts directory to PATH:
```bash
# PowerShell
...
A: 这是已修复的 #22。验证你的 wheel 是 v0.7.1（已包含 fix）/ This is #22 (already fixed). Verify your wheel is v0.7.1 (includes fix):
```bash
...
```
Pandaone-0.7.1-install.zip (210 KB)
├── dist/
│   ├── pandaone_guard-0.7.1-py3-none-any.whl   (70 KB, 推荐安装方式 / recommended install)
│   └── pandaone_guard-0.7.1.tar.gz             (146 KB, 源码包 / source archive)
└── Pandaone-0.7.1-INSTALL.md             (本文件 / this file)
```
