# PandaX v0.7.1 安装与使用指南 / PandaX v0.7.1 Installation & Usage Guide

> **历史归档**：v0.7.1 是第一次 PyPI 正式发布。修复了 v0.7.0 中 install-context 通配符阻塞 bug；首次使用 `pandax-guard` 分发名（PyPI 黑名单导致）。行为与 v0.7.0/v0.7.2 完全一致，差异在 CI 工程化。
> **Historical archive**: v0.7.1 is the first official PyPI release. Fixed the v0.7.0 install-context wildcard block bug; first used the `pandax-guard` distribution name (PyPI blacklist). Behavior fully compatible with v0.7.0/v0.7.2; differences only in CI engineering.
...
# 假设 PandaX-0.7.1-install.zip 解压到 D:\pandax-pkg\ / Assume PandaX-0.7.1-install.zip extracted to D:\pandax-pkg\
cd D:\pandax-pkg
...
# Copy PandaX-0.7.1-install.zip + deps/ to target machine together
...
├── dist/
│   ├── pandax_guard-0.7.1-py3-none-any.whl   (70 KB, 推荐安装方式 / recommended install)
│   └── pandax_guard-0.7.1.tar.gz             (146 KB, 源码包 / source archive)
└── PandaX-0.7.1-INSTALL.md             (本文件 / this file)
```
