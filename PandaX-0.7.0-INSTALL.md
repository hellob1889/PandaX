# Pandaone AI Agent v0.7.0 安装与使用指南 / Pandaone v0.7.0 Installation & Usage Guide
...
# 假设 Pandaone-0.7.0-install.zip 解压到 D:\pandaone-pkg\ / Assume Pandaone-0.7.0-install.zip extracted to D:\pandaone-pkg\
cd D:\pandaone-pkg

# 安装 wheel（pip 会自动拉依赖）/ Install wheel (pip auto-pulls dependencies)
pip install dist\pandaone-0.7.0-py3-none-any.whl

# 或一行完成 / Or one-liner
pip install pandaone-0.7.0-py3-none-any.whl
```

### 2.2 备选：从源码 sdist 安装 / Alternative: Install from Source sdist

```bash
pip install dist\pandaone-0.7.0.tar.gz
```
...
```bash
pip download pandaone-0.7.0-py3-none-any.whl -d deps/
# 会下载 watchdog / openpyxl / python-docx / reportlab / pyyaml 等
# Will download watchdog / openpyxl / python-docx / reportlab / pyyaml etc.

# 把 Pandaone-0.7.0-install.zip + deps/ 一起拷到目标电脑
# Copy Pandaone-0.7.0-install.zip + deps/ to target machine together
# 目标电脑上 / On target machine:
pip install --no-index --find-links=deps/ pandaone-0.7.0-py3-none-any.whl
```
...
```bash
pip install --no-build-isolation dist\pandaone-0.7.0.tar.gz
# 或 / Or
...
```bash
pip uninstall pandaone
```

## 八、文件清单 / File Listing

```
Pandaone-0.7.0-install.zip (210 KB)
├── dist/
│   ├── pandaone-0.7.0-py3-none-any.whl   (70 KB, 推荐安装方式 / recommended install)
│   └── pandaone-0.7.0.tar.gz             (146 KB, 源码包 / source archive)
└── Pandaone-0.7.0-INSTALL.md             (本文件 / this file)
```
